...
{	 
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM1, ENABLE);	//使能TIM2时钟
		...
		{	
			...
			{	  			
				...
				TIM1CH1_CAPTURE_VAL=TIM_GetCapture1(TIM2);
		   		TIM_OC1PolarityConfig(TIM1,TIM_ICPolarity_Falling); //CC1P=0 设置为上升沿捕获
			}else  								//还未开始,第一次捕获上升沿
			{
				...
				TIM1CH1_CAPTURE_STA|=0X40;		//标记捕获到了上升沿
		   		TIM_OC1PolarityConfig(TIM1,TIM_ICPolarity_Rising);		//CC1P=1 设置为下降沿捕获
			}		    
		}			     	    					   
 	}

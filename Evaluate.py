...
def predicting_ABP_waveform(indix: int = 0):
    ...
    dt = pickle.load(open(os.path.join(r'data', 'test.p'), 'rb'))  # loading test data
    X_test = dt['X_test']
    Y_test = dt['Y_test']

    dt = pickle.load(open(os.path.join(r'data', 'meta0.p'), 'rb'))  # loading metadata
    max_ppg = dt['max_ppg']
    min_ppg = dt['min_ppg']
    max_abp = dt['max_abp']
    min_abp = dt['min_abp']

    Y_test_pred = pickle.load(
        open('test_output.p', 'rb'))  # loading precomputed output from refinement network
    ...
    font1 = {'fontname': 'Arial', 'fontsize': 15, 'fontweight': 'bold'}
    font2 = {'fontname': 'Arial', 'fontsize': 20, 'fontweight': 'bold'}
    ax1 = plt.subplot(4, 1, 1)
    plt.plot(time_scale, ppg_signal, c='k', linewidth=2)
    plt.tick_params(bottom=True, top=False, left=True, right=False, direction='in', length=3, width=1, colors='black')
    plt.gca().set_facecolor('none')
    plt.ylabel('Amplitude(a.u.)', **font1)
    plt.title('Input PPG Signal', **font2)
    ...
    plt.tick_params(bottom=True, top=False, left=True, right=False, direction='in', length=3, width=1, colors='black')
    plt.gca().set_facecolor('none')
    plt.ylabel('mmHg', **font1)
    plt.title('Output ABP signal', **font2)
    ...
    plt.plot(time_scale, abp_signal_pred, c='b', label='Output ABP signal', linewidth=2)
    plt.plot(time_scale, abp_signal_ground_truth, c='g', label='Ground Truth', linewidth=2)
    plt.tick_params(bottom=True, top=False, left=True, right=False, direction='in', length=3, width=1, colors='black')
    plt.gca().set_facecolor('none')
    plt.title('Comparison', **font2)
...
def evaluate_BHS_Standard():
    ...
    dt = pickle.load(open(os.path.join(r'data', 'meta1.p'), 'rb'))  # loading meta data
    max_ppg = dt['max_ppg']
    min_ppg = dt['min_ppg']
    max_abp = dt['max_abp']
    min_abp = dt['min_abp']

    Y_pred = pickle.load(open(os.path.join('test_output.p'), 'rb'))  # loading prediction
    ...
    ax1.set_xlabel(r'$|$' + 'Error' + r'$|$' + ' (mmHg)', fontsize=16)
    ...
    return plt, [round(dbp_percent[0], 1), round(dbp_percent[1], 1), round(dbp_percent[2], 1)], [
        round(map_percent[0], 1), round(map_percent[1], 1), round(map_percent[2], 1)], [round(sbp_percent[0], 1),
                                                                                        round(sbp_percent[1], 1),
                                                                                        round(sbp_percent[2], 1)]
...
def evaluate_AAMI_Standard():
    ...
    font = {'fontname': 'Arial', 'fontsize': 18, 'fontweight': 'bold'}
    ## DBPS ##
    ...
    plt.tick_params(bottom=True, top=False, left=True, right=False, direction='in', length=3, width=1, colors='black')
    plt.gca().set_facecolor('none')
    plt.hist(dbps, bins=100)
    plt.xlabel('Error (mmHg)', **font)
    ...
    return plt, '| DBP | {} | {} |'.format(round(np.mean(dbps), 3), round(np.std(dbps), 3)), '| MAP | {} | {} |'.format(
        round(np.mean(maps), 3), round(np.std(maps), 3)), '| SBP | {} | {} |'.format(round(np.mean(sbps), 3),
                                                                                     round(np.std(sbps), 3))
...
def evaluate_BP_Classification():
    ...
    dt = pickle.load(open(os.path.join(r'data', 'meta0.p'), 'rb'))  # loading meta data
    max_ppg = dt['max_ppg']
    min_ppg = dt['min_ppg']
    max_abp = dt['max_abp']
    min_abp = dt['min_abp']
...
def bland_altman_plot():
    ...
    dt = pickle.load(open(os.path.join(r'data', 'meta9.p'), 'rb'))  # loading meta data
    max_ppg = dt['max_ppg']
    min_ppg = dt['min_ppg']
    max_abp = dt['max_abp']
    min_abp = dt['min_abp']
...
def regression_plot():
    ...
    dt = pickle.load(open('test.p', 'rb'))  # loading the test data

    Ytrue = dt['Y_test']

    dt = pickle.load(open(os.path.join(r'data', 'meta0.p'), 'rb'))  # loading the meta data
    max_ppg = dt['max_ppg']
    min_ppg = dt['min_ppg']
    max_abp = dt['max_abp']
    min_abp = dt['min_abp']
    ...
    plt.xlabel('Reference Value (mmHg)', **font)
    plt.ylabel('Estimated Value (mmHg)', **font)
    ax1.spines['top'].set_color('black')
    ax1.spines['right'].set_color('black')
    ax1.spines['bottom'].set_color('black')
    ax1.spines['left'].set_color('black')
    ...
    return plt, scipy.stats.linregress(dbpTrues, dbpPreds), scipy.stats.linregress(mapTrues,
                                                                                   mapPreds), scipy.stats.linregress(
        sbpTrues, sbpPreds)

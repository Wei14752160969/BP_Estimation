...
def fold_data():
    """
        folds the data into splits and saves them to perform 10-fold cross validation
    """
    ...
    for fold_id in tqdm(range(10), desc='Folding Data'):  # iterate for 10 folds
        ...
        pickle.dump({'X_train': X_train, 'Y_train': Y_train},
                    open(os.path.join('data', 'train{}.p'.format(fold_id)), 'wb'))
        ...
        pickle.dump({'X_val': X_val, 'Y_val': Y_val},
                    open(os.path.join('data', 'val{}.p'.format(fold_id)), 'wb'))
        ...
        pickle.dump({'max_ppg': max_ppg,
                     ...
                     'max_abp': max_abp,
                     'min_abp': min_abp},
                    open(os.path.join('data', 'meta{}.p'.format(fold_id)), 'wb'))

    fl = h5py.File(os.path.join('data', 'data_process.hdf5'), 'r')  # loading the episode data

    X_test = []  # intialize test data
    Y_test = []

    for i in tqdm(range(30000, len(fl['data']))):
